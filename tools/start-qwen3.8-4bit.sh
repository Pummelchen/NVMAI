#!/usr/bin/env bash
# Start NVMAI serving Qwen3.8-Flash-Next 125B-A6B at 4-bit, no questions asked.
#
...
#
#   tools/start-qwen3.8-4bit.sh [codex|qwen|opencode] [full|fast] [default|concise] [off|on]
#
...
exec "$(cd "$(dirname "$0")" && pwd)/server_launcher.sh" \
  "${1:-codex}" "${2:-full}" qwen38 4 "${3:-default}" "${4:-${NVMAI_THINKING_MODE:-off}}"
