#!/usr/bin/env bash
# Start NVMAI serving Qwen-AgentWorld 35B-A3B at 8-bit, no questions asked.
#
...
#
#   tools/start-agentworld-8bit.sh [codex|qwen|opencode] [full|fast] [default|concise] [off|on]
#
...
exec "$(cd "$(dirname "$0")" && pwd)/server_launcher.sh" \
  "${1:-codex}" "${2:-full}" agentworld 8 "${3:-default}" "${4:-${NVMAI_THINKING_MODE:-off}}"
