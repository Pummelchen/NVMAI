#!/usr/bin/env bash
# Start NVMAI serving Ornith 1.5 35B-A3B at 4-bit, no questions asked.
#
...
#
#   tools/start-ornith-4bit.sh [codex|qwen|opencode] [full|fast] [default|concise] [off|on]
#
...
exec "$(cd "$(dirname "$0")" && pwd)/server_launcher.sh" \
  "${1:-codex}" "${2:-full}" ornith 4 "${3:-default}" "${4:-${NVMAI_THINKING_MODE:-off}}"
