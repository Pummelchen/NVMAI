#!/usr/bin/env python3
"""Run benchmarks for all 3 quantizations sequentially."""
...
for quant_label, model_path in models:
    print(f"\n{'='*110}")
    print(f"STARTING BENCHMARK: {quant_label} model")
    ...
    print(f"\n{'='*110}")
    print(f"{quant_label} BENCHMARK {'COMPLETED' if result.returncode == 0 else 'FAILED'}")
    print(f"{'='*110}\n")
