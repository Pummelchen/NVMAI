...
public struct MemoryLimits: Sendable, Equatable {
    ...
    /// Active items per task. Reaching it is a modelling error, not a
    /// capacity problem, so it throws rather than evicting: silently dropping
    /// a fact the model wrote is worse than refusing to add one.
