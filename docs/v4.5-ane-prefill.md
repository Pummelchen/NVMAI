...
> `.on` when `NVMAI_PREFILL_ANE` is unset, and the runner falls back to the GPU
...
it was opt-in — nothing changed without `NVMAI_PREFILL_ANE=on`; from v4.6 it is
the default and `NVMAI_PREFILL_ANE=off` opts out.
...
`tools/golden-baseline.sh` still runs with `NVMAI_PREFILL_ANE=off`: it is the
